...
prob055' :: Integer -> Integer
prob055' limit = fromIntegral . length . filter lychrel . takeWhile (< limit) $ [1..]
  where
    lychrel = not . any palindrome . take 50 . drop 1 . iterate fsum
    palindrome (digits 10->xs) = and $ zipWith (==) (reverse xs) xs
    fsum n = (+ n) . unDigits 10 . reverse . digits 10 $ n
