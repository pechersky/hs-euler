...
prob059' :: Integer
prob059' = fromIntegral . sum . fmap ord . maximumBy (comparing (length . filter (== ' '))) . fmap decipher $ keys
  where
    vals = ['a'..'z']
    keys = [[a,b,c] | a <- vals, b <- vals, c <- vals]
    decipher = fmap chr . zipWith xor cipher . cycle . fmap ord
