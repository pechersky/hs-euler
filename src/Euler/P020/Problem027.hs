...
prob027 :: Integer
prob027 = prob027' 1000

-- naive method

prob027' :: Integer -> Integer
prob027' limit = fst . maximumBy (comparing snd)
               $ [((a * b,) . length . takeWhile (try a b)) [0 ..] |
                  a <- [(1 - limit) .. limit - 1], b <- availPrimes]
  where
    availPrimes = takeWhile (<= limit) primes
    try a b n = isPrime (n * n + a * n + b)
